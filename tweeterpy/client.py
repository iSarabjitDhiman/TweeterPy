...
class TweeterPyClient:
    def __init__(self, session: TweeterPySession, definitions: Optional[APIDefinition] = None) -> None:
    ...
    def _apply_updates(self, home_page: Union[BeautifulSoup, Any], ondemand_file_response: str, new_definitions: Dict[str, Any]):
        """Shared logic to update state from fetched data."""

        # Update API Definitions
        self.api_definitions.update(features=new_definitions.get("features", {}),
        ...
        try:
            self.session.client_transaction = ClientTransaction(
                home_page_response=parse_html(home_page), ondemand_file_response=ondemand_file_response)
        except Exception as error:
            logger.warning(f"Could not initialize ClientTransaction: {error}")

    def execute(self, operation: Union[Operation, str], variables: Optional[Dict[str, Any]] = None):
        if isinstance(operation, str):
            operation = self.api_definitions.create_operation(
                operation_name=operation, variables=variables)

        url = operation.url or f"{XUrls.GRAPHQL_BASE}/{operation.query_id}/{operation.name}"
        params = operation.payload if operation.method == "GET" else None
        json = operation.payload if operation.method == "POST" else None

        return self.session.request(url=url, method=operation.method, params=params, json=json)

    def profile_spotlights_query(self, username: str):
        variables = {"screen_name": username}
        return self.execute(operation=XOperations.ProfileSpotlightsQuery.name, variables=variables)

    def user_by_rest_id(self, user_id: str):
        variables = {"userId": user_id, "withSafetyModeUserFields": True}
        return self.execute(operation=XOperations.UserByRestId.name, variables=variables)

    def users_by_rest_ids(self, user_ids: List[str]):
        variables = {"userIds": user_ids}
        return self.execute(operation=XOperations.UsersByRestIds.name, variables=variables)

    def user_by_screen_name(self, screen_name: str):
        variables = {"screen_name": screen_name,
                     "withSafetyModeUserFields": True}
        return self.execute(operation=XOperations.UserByScreenName.name, variables=variables)

    def user_media(self, user_id: str, total: int = 100):
        variables = {"userId": user_id, "count": total, "includePromotedContent": True,
    ...
    def user_tweets_and_replies(self, user_id: str, total: int = 20):
        ...
        return self.execute(operation=XOperations.UserTweetsAndReplies.name, variables=variables)

    def tweet_result_by_rest_id(self, tweet_id: str):
        variables = {"tweetId": tweet_id, "withBirdwatchNotes": True, "withCommunity": False,
                     "includePromotedContent": False, "withVoice": False}
        return self.execute(operation=XOperations.TweetResultByRestId.name, variables=variables)

    def tweet_detail(self, tweet_id: str):
        referer = random.choice(['home', 'profile', 'tweet'])
        variables = {"focalTweetId": tweet_id, "referrer": referer, "rankingMode": "Relevance", "with_rux_injections": False, "includePromotedContent": True, "withCommunity": True,
                     "withQuickPromoteEligibilityTweetFields": True, "withArticleRichContent": False, "withBirdwatchNotes": True, "withVoice": True, "withV2Timeline": True}
    ...
    def home_timeline(self, total: int = 40):
        variables = {"count": total, "includePromotedContent": True,
                     "latestControlAvailable": True, "withCommunity": True}
        return self.execute(operation=XOperations.HomeTimeline.name, variables=variables)

    def list_latest_tweets_timeline(self, list_id: str, total: int = 100):
        variables = {"listId": list_id, "count": total}
        return self.execute(operation=XOperations.ListLatestTweetsTimeline.name, variables=variables)

    def topic_landing_page(self, topic_id: str, total: int = 100):
        variables = {"rest_id": topic_id, "count": total}
        return self.execute(operation=XOperations.TopicLandingPage.name, variables=variables)

    def followers(self, user_id: str, total: int = 100):
        variables = {"userId": user_id, "count": total,
    ...
    def followers_you_know(self, user_id: str, total: int = 100):
        variables = {"userId": user_id, "count": total,
                     "includePromotedContent": False}
        return self.execute(operation=XOperations.FollowersYouKnow.name, variables=variables)

    def biz_profile_fetch_user(self, user_id: int):
        variables = {"rest_id": user_id}
        return self.execute(operation=XOperations.BizProfileFetchUser.name, variables=variables)

    def favoriters(self, tweet_id: str, total: int = 100):
        variables = {"tweetId": tweet_id, "count": total,
    ...
    def user_highlights_tweets(self, user_id: int, total: int = 100):
        ...
        return self.execute(operation=XOperations.UserHighlightsTweets.name, variables=variables)

    def search(self, query: str, search_filter: str = "Top", total: int = 20):
        variables = {"rawQuery": query, "count": total,
                     "querySource": "typed_query", "product": search_filter}
        return self.execute(operation=XOperations.SearchTimeline.name, variables=variables)


class TweeterPy(TweeterPyClient):
    def __init__(self, session: Optional[TweeterPySession] = None, definitions: Optional[APIDefinition] = None) -> None:
        if session is None:
            session = Session()
    ...
    def initialize(self, deep_scan: bool = False):
        ...
        new_definitions = self.updater.run(
        ...
        self._apply_updates(home_page=home_page, ondemand_file_response=str(
            ondemand_file_response), new_definitions=new_definitions)
...
class TweeterPyAsync(TweeterPyClient):
    ...
    async def initialize(self, deep_scan: bool = False):
        ...
        # Dynamic API Definitions Update
        new_definitions = await self.updater.run_async(response=str(home_page), deep_scan=deep_scan)

        # ClientTransaction Bundle
        ondemand_s_bundle_file = self.parser.get_bundle_url(
            bundle_name="ondemand.s", html_content=home_page)
        ondemand_file_response = await self.session.request_html(url=ondemand_s_bundle_file, method="GET")
