...
class TaskHandler:
    ...
    def _create_task_mapper(self, username, password, verification_input_data):
        ...
        task_flow_mapper = {
            "LoginJsInstrumentationSubtask": {
                "task_executor": self._get_user_flow_token,
                "task_parameter": None,
            },
            "LoginEnterUserIdentifierSSO": {
                "task_executor": self._get_password_flow_token,
                "task_parameter": username,
            },
            "LoginEnterAlternateIdentifierSubtask": {
                "task_executor": self._handle_suspicious_login,
                "task_parameter": verification_input_data,
            },
            "LoginEnterPassword": {
                "task_executor": self._get_account_duplication_flow_token,
                "task_parameter": password,
            },
            "DenyLoginSubtask": {
                "task_executor": self._check_suspicious_login,
                "task_parameter": None,
            },
            "AccountDuplicationCheck": {
                "task_executor": self._check_account_duplication,
                "task_parameter": None,
            },
            "LoginAcid": {
                "task_executor": self._handle_suspicious_login,
    ...
    def _get_flow_token(self):
        ...
        payload = {
            "input_flow_data": {
                "flow_context": {
                    "debug_overrides": {},
                    "start_location": {"location": login_location},
                },
            },
            "subtask_versions": {
                "action_list": 2,
        ...
        return self.request_client.request(
            Path.TASK_URL, method="POST", params=params, json=payload
        )

    def _get_javscript_instrumentation_subtask(self):
        params = {"c_name": "ui_metrics"}
        return self.request_client.request(
            Path.JAVSCRIPT_INSTRUMENTATION_URL, params=params
        )

    def _get_user_flow_token(
        self, flow_token, subtask_id="LoginJsInstrumentationSubtask"
    ):
        payload = {
    ...
    @disable_logger
    def _get_password_flow_token(
        self, flow_token, subtask_id="LoginEnterUserIdentifierSSO", username=None
    ):
        payload = {
            "flow_token": flow_token,
            "subtask_inputs": [
    ...
    @disable_logger
    def _get_account_duplication_flow_token(
        self, flow_token, subtask_id="LoginEnterPassword", password=None
    ):
        payload = {
            "flow_token": flow_token,
            "subtask_inputs": [
    ...
    def _check_suspicious_login(self, flow_token, subtask_id="DenyLoginSubtask"):
        payload = {
            "flow_token": flow_token,
            "subtask_inputs": [
                {"subtask_id": subtask_id, "cta": {"link": "next_link"}}
            ],
        }
        return self.request_client.request(Path.TASK_URL, method="POST", json=payload)

    def _check_account_duplication(
        self, flow_token, subtask_id="AccountDuplicationCheck"
    ):
        payload = {
            "flow_token": flow_token,
            "subtask_inputs": [
    ...
    def _handle_suspicious_login(
        self, flow_token, subtask_id="LoginAcid", verification_input_data=None
    ):
        payload = {
            "flow_token": flow_token,
            "subtask_inputs": [
        ...
        while handle_incorrect_input:
            response = self.request_client.request(
                Path.TASK_URL, method="POST", json=payload, skip_error_checking=True
            )
    ...
    @disable_logger
    def login(
        self, username, password, email=None, phone=None, mfa_secret=None, **kwargs
    ):
        response = None
        error_message = None
        tasks_pending = True
        ...
        otp_code = None
        try:
            task_flow_mapper = self._create_task_mapper(
                username or email, password, verification_input_data
            )
            ...
            while tasks_pending:
                ...
                tasks = [
                    task_id
                    for task_id in task_flow_mapper.keys()
                    if task_id in response_tasks
                ]
                if tasks:
                    task_id = tasks[0]
                else:
                    ...
                    login_error = (
                        f"{(error_message or '')}{missing_task_ids_error}".strip()
                    )
                ...
                if task:
                    error_message = (
                        "\n".join(
                            [
                                x
                                for x in find_nested_key(response, "text")
                    ...
                    if task_id in [
                        ...
                        input_type = (
                            (find_nested_key(response, "keyboard_type") or "")
                            .strip()
                            .lower()
                        )
                        hint_message = (
                            (find_nested_key(response, "hint_text") or "")
                            .strip()
                            .lower()
                        )
                        input_message = (
                            f"{hint_message} (Input Type - {input_type}) ==> "
                        )
                        otp_required = (
                            True
                            if hint_message == "confirmation code"
                            and input_type == "text"
                            else False
                        )
                        email_verification = (
                        ...
                        phone_verification = (
                            True
                            if hint_message == "phone number"
                            and input_type == "telephone"
                            else False
                        )
                        identity_verification = (
                            True
                            if hint_message == "phone or username"
                            and input_type == "text"
                            else False
                        )
                        two_fac_auth = (
                        ...
                        if input_type and hint_message and error_message:
                            ...
                            verification_input_data = (
                                phone
                                if (phone_verification or identity_verification)
                                and phone
                                else email
                                if (email_verification and email)
                                else otp_code
                                if (two_fac_auth and otp_code)
                                else str(input(input_message))
                                if two_fac_auth or otp_required
                                else None
                            )
                    ...
                    if task["task_parameter"] is None:
                        ...
                        response = task.get("task_executor")(
                            flow_token, task_id, parameter
                        )
                else:
                    print(f"\n{response}")
                    tasks_pending = False
