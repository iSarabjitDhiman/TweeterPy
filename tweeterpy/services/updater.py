...
class APIUpdater:
    ...
    def _prepare_queue(self, response: str, deep_scan: bool) -> tuple[dict, dict]:
        ...
        bundle_manifest = self.parser.parse_bundle_manifest(
        ...
        if features:
            ...
            feature_switches = {feature_name: feature_switch.get("value", None) if isinstance(
                feature_switch, dict) else feature_switch for feature_name, feature_switch in raw_feature_switches.items()}
        ...
        if deep_scan and bundle_manifest:
            ...
            if operational_bundles:
                bundle_queue.update({bundle.get("bundle_name"): bundle.get(
                    "bundle_url") for bundle in operational_bundles if isinstance(bundle, dict)})
        ...
        try:
            ...
            for bundle_name, bundle_url in bundle_queue.items():
                try:
                    ...
                    js_content = self.session.request(
                        url=bundle_url, method="GET")
    ...
    async def run_async(self, response: str, deep_scan: bool = False):
        ...
        try:
            ...
            logger.info(
                f"Processing {len(bundle_queue)} bundle/s concurrently...")

            # Create all tasks for concurrent execution
            tasks = [self.fetch_bundle(bundle_name=bundle_name, bundle_url=bundle_url)
                     for bundle_name, bundle_url in bundle_queue.items()]
